...
const generateAbout = aboutText => {
    ...
    return `
        <section class="my-3" id="about">
            <h2 class="text-dark bg-priimary p-2 display-inline-block">About Me</h2>
            <p>${aboutText}</p>
        </section>
        `;
};
...
//generateProjects function before refactor
/*
const generateProjects = projectsArr => {
    ...
    const nonFeaturedProjectHtmlArr = nonFeaturedProjects.map(({ name, description, languages, link }) => {
        return `
            <div class="col-12 col-md-6 mb-2 bg-dark text-light p-3 flex-column">
                ...
                <p>${description}</p>
                <a href="${link}" class="btn mt-auto"><i class="fab fa-github mr-2"></i>View Projects on Github</a>
            </div>
            `;
    });
